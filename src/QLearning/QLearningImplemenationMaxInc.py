    ...
    for trial in range(trials):
        ...
        while (step <= max_steps_per_episode):
            ...
            if (current_state[0] - 1 >= 0):
                reward_left = MaxInc(state_space[current_state[0]][current_state[1]].users, state_space[current_state[0] - 1][current_state[1]].users)
                rewards[4] = reward_left
                if reward_left > max_reward:
                    max_reward = reward_left
                    max_reward_state = 4
            if (current_state[0] + 1 <= grid_size - 1):
                reward_right = MaxInc(state_space[current_state[0]][current_state[1]].users, state_space[current_state[0] + 1][current_state[1]].users)
                rewards[2] = reward_right
                if reward_right > max_reward:
                    max_reward = reward_right
                    max_reward_state = 2
            if (current_state[1] + 1 <= grid_size - 1):
                reward_bottom = MaxInc(state_space[current_state[0]][current_state[1]].users, state_space[current_state[0]][current_state[1] + 1].users)
                rewards[3] = reward_bottom
                if reward_bottom > max_reward:
                    max_reward = reward_bottom
                    max_reward_state = 3
            if (current_state[1] - 1 >= 0):
                reward_top = MaxInc(state_space[current_state[0]][current_state[1]].users, state_space[current_state[0]][current_state[1] - 1].users)
...
plt.title('Rewards per Episode during Q-Learning in MaxInc')
...
plt.title('Distribution of Q-Values in the Q-Table  in MaxInc')
plt.grid(True)
plt.tight_layout()
plt.show()
