...
# Create confusion matrix data
confusion_matrix_data = [[avg_TP, avg_FP], [avg_FN, avg_TN]]

# Create and display the heatmap (corrected format specifier)
sns.heatmap(confusion_matrix_data, annot=True, fmt=".2f")  # Use '.2f' for 2 decimal places
...
plt.bar(range(len(F1_score_list)), F1_score_list)
plt.xlabel('User ID')
plt.ylabel('F1-Score')
plt.title('F1-Score Distribution across Users')
plt.grid(True)
plt.tight_layout()
plt.show()
plt.show()